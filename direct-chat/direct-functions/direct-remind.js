...
function setReminder(chatId, time, message) {
    const now = Date.now();
    ...
    reminders.set(reminderId, { timeout, chatId });
...
function getMessage(text, chatId, botInstance) {
    bot = botInstance;
    ...
    const success = setReminder(chatId, parsed.time, parsed.message);
